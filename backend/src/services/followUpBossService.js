...
const buildLeadTags = (submission, extraTags = []) => {
  const leadType = submission.lead_type || submission.leadType;
  const tags = ['Website Lead', 'saahomes-organic', ...extraTags];
  if (leadType) tags.push(leadType);
  return [...new Set(tags)];
};
...
export const forwardContactToFollowUpBoss = async (submission) => {
  ...
  const eventData = {
    ...
    person: {
      ...
      tags: buildLeadTags(submission, ['contact']),
    },
  };
...
export const forwardChfaLeadToFollowUpBoss = async (submission) => {
  ...
  const eventData = {
    ...
    person: {
      ...
      emails: email ? [{ value: email, type: 'work' }] : [],
      phones: phone ? [{ value: phone.replace(/\D/g, ''), type: 'mobile' }] : [],
      tags: buildLeadTags(submission, ['CHFA Schools To Home', 'School Employee Lead', 'chfa']),
    },
  };
...
export const forwardChampionsLeadToFollowUpBoss = async (submission) => {
  ...
  const eventData = {
    ...
    person: {
      ...
      emails: email ? [{ value: email, type: 'work' }] : [],
      phones: phone ? [{ value: phone.replace(/\D/g, ''), type: 'mobile' }] : [],
      tags: buildLeadTags(submission, ['Champions Home Loan', 'First Responder Lead', 'chfa']),
    },
  };
...
export const forwardChfaDpaLeadToFollowUpBoss = async (submission) => {
  ...
  const eventData = {
    ...
    person: {
      ...
      emails: email ? [{ value: email, type: 'work' }] : [],
      phones: phone ? [{ value: phone.replace(/\D/g, ''), type: 'mobile' }] : [],
      tags: buildLeadTags(submission, ['CHFA Down Payment Assistance', 'First Time Homebuyer Lead', 'chfa']),
    },
  };
...
export const forwardGhopeLeadToFollowUpBoss = async (submission) => {
  ...
  const eventData = {
    ...
    person: {
      ...
      emails: email ? [{ value: email, type: 'work' }] : [],
      phones: phone ? [{ value: phone.replace(/\D/g, ''), type: 'mobile' }] : [],
      tags: buildLeadTags(submission, ['G-HOPE Greeley', 'Greeley Down Payment Assistance', 'chfa']),
    },
  };
...
export const forwardMarketReportToFollowUpBoss = async (submission) => {
  ...
  const eventData = {
    ...
    person: {
      ...
      emails: email ? [{ value: email, type: 'work' }] : [],
      phones: phone ? [{ value: phone.replace(/\D/g, ''), type: 'mobile' }] : [],
      tags: buildLeadTags(submission, ['Market Report Request', 'market-report']),
    },
  };
